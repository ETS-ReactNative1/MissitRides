...
export default class Ride extends React.Component {
  ...
  componentDidMount() {
    // this.setState({ timer: 1 + this.bitCount(this.state.pickup.key) + this.bitCount(this.state.dropoff.key) + 2 })
    this.getRates();
  ...
  updateRegion = () => {
    const delta = getDistanceInDegs(
      this.state.pickup.latlong.latitude,
      this.state.pickup.latlong.longitude,
      this.state.dropoff.latlong.latitude,
      this.state.dropoff.latlong.longitude);
    return {
                  ...
                  <Pressable key={ride["key"]} style={[rideStyles.card, this.state.ride === ride["key"] ? { backgroundColor: '#d9d9d9' } : null]}
                    ...
                    <Block style={{ justifyContent: 'center', alignItems: 'center' }}>
                      <Image
                        ...
                        source={ride["passengers"] > 4 ? require('../assets/icons/big.png') :
                          ride["class"] == "Standard" ? require('../assets/icons/std.png') :
                            require('../assets/icons/fancy.png')}
                      />
                    </Block>
                    <Block style={{ flex: 2 }}>
